  ...
  rect = (function() {
    ...
    rect.bounding_vecs = function(vec_array) {
      ...
      for (i = _i = 0, _len = vec_array.length; _i < _len; i = ++_i) {
        v = vec_array[i];
        if ((i === 0) || v.x < res.ll.x) res.ll.x = v.x;
        if ((i === 0) || v.x > res.ur.x) res.ur.x = v.x;
        if ((i === 0) || v.y < res.ll.y) res.ll.y = v.y;
        if ((i === 0) || v.y > res.ur.x) res.ur.y = v.y;
      }
      return res;
    };
    ...
    rect.bounding_rects = function(rects_array) {
      ...
      if (!rects_array.length) {
        ...
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          r = _ref[_i];
          if (r.ll.x < res.ll.x) res.ll.x = r.ll.x;
          if (r.ur.x > res.ur.x) res.ur.x = r.ur.x;
          if (r.ll.y < res.ll.y) res.ll.y = r.ll.y;
          if (r.ur.y > res.ur.y) res.ur.y = r.ur.y;
        }
        return res;
      }
    };
